...
export const NoticeGrid = (props: GridProps) => {
    ...
    const toLocalDateTimeString = (date: string | Date) => {
        ...
        // 한국 시간대(Asia/Seoul) 기준으로 연-월-일 시:분:초 형식으로 변환
    ...
    const deleteRowButtonClicked = async (id: GridRowId) => {
        ...
        const response = await deleteNotice(rowToDelete.seq)
    ...
    const updateRowProcess = async (newRow: GridNoticeItem) => {
        if (newRow.title === '' || newRow.category === '') {
        ...
        const formattedExpiredDate = toLocalDateTimeString(newRow.expiredAt)
        if (newRow.isNew) {
